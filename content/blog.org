...
***** Tip with some Sats
[[https://getalby.com/p/tacosandlinux][Tip Some Sats ⚡]]
** TODO My new Laptop. The Framework Laptop
:PROPERTIES:
:EXPORT_FILE_NAME: my-new-laptop-dot-the-framework-laptop
:END:
** TODO Migrate a KVM Virtual Machine
:PROPERTIES:
:EXPORT_FILE_NAME: migrate-a-kvm-virtual-machine
:END:
*** Connect to Remote machine via Virt Mananager
In order to do this with a regular user, make sure that your regular user is part of the Libvirt Group. If not try this:
#+begin_src shell
sudo usermod -a -G libvirt $(whoami)
#+end_src
Also make sure that if you are connecting via SSH and you do not have SSH key pairs configured that you have the package ~openssh-askpass~ installed. In GNOME this prompts for the password of the remote system. Virt-Manger by default work with SSH keys which is helpful to keep things more secure.
*** Shared Storage
One thing that I kept getting wrong about how VM migrations worked in KVM is that tools like Virt Manager literally only manage the VM. Resources for each VM have to be carefully setup in order to create a robust and performant VM environment. If you are creating VMs solely on a single workstation then those VMs are basically tied to that workstation. The concept of Migration in Virt Manager basically means moving the management of a VM from one Virt Manager to another. You are not actually moving the underlying Virtual Machine "image". You are simply moving all it's configurations and settings to another Virt-Manager instance.

You can move a virtual machine image from one computer to another and import all it's Virt-manager configurations and settings. But this is not what is refered as a migration in Virt-manager.
** TODO Cybersecurity Analyst Learns Automation
:PROPERTIES:
:EXPORT_FILE_NAME: cybersecurity-analyst-learns-automation
:END:
*** Why Automation?
Automation has many meanings in the technology sphere and various uses in day to day work. Why I decided to learn automation was because Cybersecurity work is evolving. The number and intensity of attacks is not slowing down. The day to day work is not becoming streamlined, it's actually becoming increasing chaotic. To work in Cybersecurity you need Discipline. As author and former Navy Seal states: "Discipline equals Freedom". I want freedom from constant warnings and alerts that simply increase noise and anxiety.
*** Benefits of Automation
Writing a powershell script to output a csv file of users who's password are about to expire is NOT automation. But that powershell script is a part of automation. I will be upfront and honesty. Automation is hard and it need constant tending. It's much like gardening in the sense that you work the land and tend to the plants. After all the hard work you put into your garden you get a beautiful garden full of plants. Automation will require many different components and much work to get all those pieces to work together.
*** The Basics
The basics of Automation start with trying to solve the everyday annoying problems. Finding clever and effecient solutions to sovling these problems give you back hundreds of hours. Earlier I mentiond a powershell script to get a list of users whose passwords are about to expire. Alone this information is quite helpful, but what action will you need to take on this information? Well maybe emailing them a day or two before to change their passwords CC'ing their managers may prompt them to actually update their passwords. Luckily, Powershell alone can do all of these actions.
***** Thank You
